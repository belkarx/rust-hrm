...
fn init_person() -> Person {
    Person {
        ...
        other_contacts: {
            let other_contacts:Option<Vec<String>> = Some(get_string("Other contacts (comma separated) [optional]: ").split(", ").map(|s| s.to_string()).collect()).filter(|s:Vec<String>| s.len() > 0);
            other_contacts
